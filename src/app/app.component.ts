...
import {Info} from './services/info.interface';
...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apiService.getAll('user', 'EnzDev').subscribe((info: Info) => {
      this.userDescription = info.description;
      this.overviewData = [
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const language = info.topLanguages[i];
        languagesRows.push({
          title: language.name,
          data: language.repos
        });
